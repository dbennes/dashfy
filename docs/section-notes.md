...
Each dashboard section (S00–S05) has a compact comment bar. **Registrar** opens
the form; **Histórico** lists its records, filters status and paginates 20 at a
time. Initials represent distinct comment authors, not live online presence.
The dashboard header exports **DASHFY_Ata.xlsx**, containing one worksheet with
...
Comments begin as **Pendente**. A note date and text (up to 4,000 characters) are
required. A forecast completion date is required unless **Somente informação** is
...
The original note and author remain unchanged. **Sanado**, **Cancelado** and a
return to **Pendente** each append an event containing the previous/new status,
...
Pendente is also audited. Optimistic versions reject concurrent stale changes;
